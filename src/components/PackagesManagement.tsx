import { useState, useEffect } from 'react';
import { API_ENDPOINTS } from '../config/api';
import { toast } from 'react-hot-toast';

interface Package {
  _id: string;
  name: string;
  amount: number;
  description?: string;
  isActive: boolean;
  isVip?: boolean;
  whatsappGroupLink?: string;
  userCount?: number;
}

interface PackagesManagementProps {
    mode?: 'all' | 'vip' | 'standard';
}

export default function PackagesManagement({ mode = 'all' }: PackagesManagementProps) {
  const [packages, setPackages] = useState<Package[]>([]);
  const [loading, setLoading] = useState(false);
  const [showAddModal, setShowAddModal] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [newPackage, setNewPackage] = useState({
    name: '',
    amount: '',
    description: '',
    isVip: mode === 'vip', // Default to VIP if mode is VIP
    whatsappGroupLink: '',
  });

  // Members View State
  const [viewingMembersPkg, setViewingMembersPkg] = useState<Package | null>(null);
  const [members, setMembers] = useState<any[]>([]);
  const [loadingMembers, setLoadingMembers] = useState(false);

  const fetchPackages = async () => {
    setLoading(true);
    try {
      const response = await fetch(API_ENDPOINTS.PACKAGES.LIST);
      const data = await response.json();
      if (data.success) {
        setPackages(data.data);
      }
    } catch (err) {
      toast.error('Failed to load packages');
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchPackages();
  }, []);

  useEffect(() => {
    setNewPackage(prev => ({ ...prev, isVip: mode === 'vip' }));
  }, [mode]);

  const handleViewMembers = async (pkg: Package) => {
    setViewingMembersPkg(pkg);
    setLoadingMembers(true);
    try {
        const token = localStorage.getItem('token');
        const res = await fetch(API_ENDPOINTS.USERS.GET_ALL, {
            headers: { Authorization: `Bearer ${token}` }
        });
        const data = await res.json();
        if (data.success && Array.isArray(data.data)) {
            // Filter users by package amount
            // Users have 'package' field which is the amount number
            const pkgMembers = data.data.filter((u: any) => Number(u.package) === Number(pkg.amount));
            setMembers(pkgMembers);
        } else {
            setMembers([]);
            toast.error('Failed to load members');
        }
    } catch (err) {
        toast.error('Network error');
    } finally {
        setLoadingMembers(false);
    }
  };

  const handleEdit = (pkg: Package) => {
    setNewPackage({
        name: pkg.name,
        amount: pkg.amount.toString(),
        description: pkg.description || '',
        isVip: pkg.isVip || false,
        whatsappGroupLink: (pkg as any).whatsappGroupLink || ''
    });
    setEditingId(pkg._id);
    setShowAddModal(true);
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newPackage.name || !newPackage.amount) {
      toast.error('Name and Amount are required');
      return;
    }

    try {
      const token = localStorage.getItem('token');
      const url = editingId 
        ? API_ENDPOINTS.PACKAGES.UPDATE(editingId)
        : API_ENDPOINTS.PACKAGES.CREATE;
      
      const method = editingId ? 'PUT' : 'POST';

      const response = await fetch(url, {
        method,
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        },
        body: JSON.stringify({
          ...newPackage,
          amount: Number(newPackage.amount),
        }),
      });

      const data = await response.json();
      if (data.success) {
        toast.success(editingId ? 'Package updated' : 'Package created');
        setShowAddModal(false);
        setNewPackage({ name: '', amount: '', description: '', isVip: false, whatsappGroupLink: '' });
        setEditingId(null);
        fetchPackages();
      } else {
        toast.error(data.message || 'Operation failed');
      }
    } catch (err) {
      toast.error('Network error');
    }
  };

  const handleDelete = async (id: string) => {
    if (!window.confirm('Are you sure you want to delete this package?')) return;

    try {
      const token = localStorage.getItem('token');
      const response = await fetch(API_ENDPOINTS.PACKAGES.DELETE(id), {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      const data = await response.json();
      if (data.success) {
        toast.success('Package deleted successfully');
        fetchPackages();
      } else {
        toast.error(data.message || 'Failed to delete package');
      }
    } catch (err) {
      toast.error('Network error while deleting package');
    }
  };

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-bold text-white tracking-tight text-left">
            {mode === 'vip' ? 'VIP Packages' : mode === 'standard' ? 'Standard Packages' : 'Registration Packs'}
          </h2>
          <p className="text-slate-300 mt-1 text-left">Manage contest entry fees and tiers</p>
        </div>
        <button
          onClick={() => {
            setEditingId(null);
            setNewPackage({ name: '', amount: '', description: '', isVip: mode === 'vip', whatsappGroupLink: '' });
            setShowAddModal(true);
          }}
          className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-blue-600/30 flex items-center gap-2"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
          </svg>
          Add {mode === 'vip' ? 'VIP' : ''} Pack
        </button>
      </div>

      {['all', 'vip'].includes(mode) && (
        <div className="mb-12">
          {mode === 'all' && (
            <h3 className="text-xl font-black text-amber-500 uppercase tracking-widest mb-6 flex items-center gap-2">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>
                VIP Packages
            </h3>
          )}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {loading && packages.length === 0 ? (
                  <div className="col-span-full py-10 text-center">
                      <div className="inline-block w-8 h-8 border-4 border-amber-500/30 border-t-amber-500 rounded-full animate-spin"></div>
                  </div>
              ) : packages.filter(p => p.isVip).length === 0 ? (
                  <div className="col-span-full py-10 bg-amber-500/5 rounded-3xl border border-amber-500/10 text-center border-dashed">
                      <p className="text-amber-500/50 font-bold uppercase tracking-widest text-xs">No VIP packages created yet</p>
                  </div>
              ) : (
                  packages.filter(p => p.isVip).map((pkg) => (
                      <div
                      key={pkg._id}
                      className="bg-gradient-to-br from-amber-500/10 to-yellow-500/5 backdrop-blur-lg border border-amber-500/20 p-6 rounded-3xl transition-all group relative hover:border-amber-500/50 flex flex-col justify-between"
                      >
                       <div>
                        <div className="absolute top-3 right-3 bg-gradient-to-r from-amber-400 to-yellow-500 text-white text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md flex items-center gap-1 shadow-lg shadow-amber-500/20">
                            <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                            </svg>
                            VIP
                        </div>
                        
                        <div className="flex justify-between items-start mb-4 text-left">
                            <div>
                            <h3 className="text-xl font-black text-white uppercase tracking-tight">{pkg.name}</h3>
                            <p className="text-amber-200/60 text-xs mt-1">{pkg.description || 'No description'}</p>
                            {(pkg as any).whatsappGroupLink && (
                                <a 
                                    href={(pkg as any).whatsappGroupLink} 
                                    target="_blank" 
                                    rel="noopener noreferrer"
                                    className="inline-flex items-center gap-1 mt-2 text-green-400 hover:text-green-300 text-xs font-bold transition-colors"
                                >
                                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.417-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.652zm6.599-3.835c1.474.875 3.183 1.357 4.935 1.358 4.833.002 8.767-3.931 8.77-8.766 0-2.343-.911-4.544-2.566-6.199s-3.855-2.566-6.198-2.567c-4.835 0-8.771 3.935-8.774 8.77-.001 1.573.411 3.103 1.196 4.453l1.01 1.742-1.071 3.91 4.008-1.051zm10.946-6.166c-.103-.173-.38-.277-.796-.485s-2.459-1.213-2.839-1.353-.657-.208-.933.208-.103.208-.069.277.103.173.208.277c.103.104.208.242.311.346.103.104.242.242.103.485s-.208.242-.484.242-.276.104-1.972-.519c-1.319-1.177-2.208-2.632-2.467-3.048s-.027-.64.18-.847c.187-.186.415-.484.622-.726.208-.242.276-.415.415-.691s.069-.519-.034-.726-.933-2.248-1.279-3.078c-.337-.813-.68-.703-.933-.716s-.519-.013-.795-.013-.726.104-1.107.519-1.453 1.419-1.453 3.46 1.487 4.012 1.694 4.288c.208.277 2.925 4.46 7.087 6.256 1.13.487 1.83.649 2.505.862.991.315 1.898.27 2.613.163.796-.118 2.316-1.141 2.645-2.248s.329-2.041.208-2.248z" /></svg>
                                    Join Group
                                </a>
                            )}
                            </div>
                            <div className="px-3 py-1 rounded-lg font-black text-lg bg-amber-500/20 text-amber-400">
                            ₹{pkg.amount.toLocaleString()}
                            </div>
                        </div>
                       </div>

                      <div className="flex items-center gap-4 mt-6">
                          <div className="flex-1 text-left cursor-pointer hover:opacity-80" onClick={() => handleViewMembers(pkg)}>
                            <p className="text-[10px] font-black text-amber-500/50 uppercase tracking-widest">Registered Users</p>
                             <div className="flex items-center gap-2">
                                <p className="text-white font-black text-xl">{pkg.userCount || 0}</p>
                                <span className="text-xs text-amber-500 underline">View List</span>
                             </div>
                          </div>
                          <div className="flex gap-2">
                              <button
                                  onClick={() => handleEdit(pkg)}
                                  className="p-3 bg-amber-500/10 hover:bg-amber-500 text-amber-500 hover:text-white rounded-xl transition-all"
                              >
                                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                              </button>
                              <button
                              onClick={() => handleDelete(pkg._id)}
                              className="p-3 bg-red-600/10 hover:bg-red-600 text-red-500 hover:text-white rounded-xl transition-all"
                              >
                              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                              </svg>
                              </button>
                          </div>
                      </div>
                      </div>
                  ))
              )}
          </div>
        </div>
      )}

      {['all', 'standard'].includes(mode) && (
        <div>
          {mode === 'all' && (
            <h3 className="text-xl font-black text-blue-400 uppercase tracking-widest mb-6 flex items-center gap-2">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
                Standard Packages
            </h3>
          )}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {loading && packages.length === 0 ? (
              <div className="col-span-full py-20 text-center">
                  <div className="inline-block w-12 h-12 border-4 border-white/10 border-t-blue-500 rounded-full animate-spin"></div>
              </div>
              ) : packages.filter(p => !p.isVip).length === 0 ? (
              <div className="col-span-full py-20 bg-white/5 rounded-3xl border border-white/10 text-center">
                  <p className="text-slate-400 font-bold uppercase tracking-widest">No standard packages found</p>
              </div>
              ) : (
              packages.filter(p => !p.isVip).map((pkg) => (
                  <div
                  key={pkg._id}
                  className="bg-white/5 backdrop-blur-lg border border-white/10 p-6 rounded-3xl transition-all group relative hover:border-blue-500/50 flex flex-col justify-between"
                  >
                   <div>
                    <div className="flex justify-between items-start mb-4 text-left">
                        <div>
                        <h3 className="text-xl font-black text-white uppercase tracking-tight">{pkg.name}</h3>
                        <p className="text-slate-400 text-xs mt-1">{pkg.description || 'No description'}</p>
                        {(pkg as any).whatsappGroupLink && (
                            <a 
                                href={(pkg as any).whatsappGroupLink} 
                                target="_blank" 
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-1 mt-2 text-green-400 hover:text-green-300 text-xs font-bold transition-colors"
                            >
                                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.417-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.652zm6.599-3.835c1.474.875 3.183 1.357 4.935 1.358 4.833.002 8.767-3.931 8.77-8.766 0-2.343-.911-4.544-2.566-6.199s-3.855-2.566-6.198-2.567c-4.835 0-8.771 3.935-8.774 8.77-.001 1.573.411 3.103 1.196 4.453l1.01 1.742-1.071 3.91 4.008-1.051zm10.946-6.166c-.103-.173-.38-.277-.796-.485s-2.459-1.213-2.839-1.353-.657-.208-.933.208-.103.208-.069.277.103.173.208.277c.103.104.208.242.311.346.103.104.242.242.103.485s-.208.242-.484.242-.276.104-1.972-.519c-1.319-1.177-2.208-2.632-2.467-3.048s-.027-.64.18-.847c.187-.186.415-.484.622-.726.208-.242.276-.415.415-.691s.069-.519-.034-.726-.933-2.248-1.279-3.078c-.337-.813-.68-.703-.933-.716s-.519-.013-.795-.013-.726.104-1.107.519-1.453 1.419-1.453 3.46 1.487 4.012 1.694 4.288c.208.277 2.925 4.46 7.087 6.256 1.13.487 1.83.649 2.505.862.991.315 1.898.27 2.613.163.796-.118 2.316-1.141 2.645-2.248s.329-2.041.208-2.248z" /></svg>
                                Join Group
                            </a>
                        )}
                        </div>
                        <div className="px-3 py-1 rounded-lg font-black text-lg bg-blue-600/20 text-blue-400">
                        ₹{pkg.amount.toLocaleString()}
                        </div>
                    </div>
                  </div>

                  <div className="flex items-center gap-4 mt-6">
                      <div className="flex-1 text-left cursor-pointer hover:opacity-80" onClick={() => handleViewMembers(pkg)}>
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Registered Users</p>
                        <div className="flex items-center gap-2">
                           <p className="text-white font-black text-xl">{pkg.userCount || 0}</p>
                           <span className="text-xs text-blue-500 underline">View List</span>
                        </div>
                      </div>
                      <div className="flex gap-2">
                          <button
                              onClick={() => handleEdit(pkg)}
                              className="p-3 bg-blue-600/10 hover:bg-blue-600 text-blue-500 hover:text-white rounded-xl transition-all"
                          >
                              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                          </button>
                          <button
                          onClick={() => handleDelete(pkg._id)}
                          className="p-3 bg-red-600/10 hover:bg-red-600 text-red-500 hover:text-white rounded-xl transition-all"
                          >
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                          </svg>
                          </button>
                      </div>
                  </div>
                  </div>
              ))
              )}
          </div>
        </div>
      )}

      {/* Add/Edit Package Modal */}
      {showAddModal && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl flex items-center justify-center z-[110] p-4 text-left">
          <div className="bg-slate-900 border border-white/10 rounded-3xl p-8 max-w-md w-full shadow-2xl">
            <h3 className="text-2xl font-black text-white mb-6 uppercase tracking-tight">
                {editingId ? 'Edit Pack' : 'Create New Pack'}
            </h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 mb-2 block">
                  Pack Name
                </label>
                <input
                  type="text"
                  value={newPackage.name}
                  onChange={(e) => setNewPackage({ ...newPackage, name: e.target.value })}
                  placeholder="e.g. Silver Tier"
                  className="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-xl text-white font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all shadow-inner"
                />
              </div>
              <div>
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 mb-2 block">
                  Amount (₹)
                </label>
                <input
                  type="number"
                  value={newPackage.amount}
                  onChange={(e) => setNewPackage({ ...newPackage, amount: e.target.value })}
                  placeholder="500"
                  className="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-xl text-white font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all shadow-inner"
                />
              </div>
              <div>
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 mb-2 block">
                  Description
                </label>
                <textarea
                  value={newPackage.description}
                  onChange={(e) => setNewPackage({ ...newPackage, description: e.target.value })}
                  placeholder="Optional details"
                  rows={3}
                  className="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-xl text-white font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all shadow-inner resize-none"
                />
              </div>

               {/* WhatsApp Link Input */}
              <div>
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 mb-2 block">
                  WhatsApp Group Link (Optional)
                </label>
                <input
                  type="url"
                  value={newPackage.whatsappGroupLink}
                  onChange={(e) => setNewPackage({ ...newPackage, whatsappGroupLink: e.target.value })}
                  placeholder="https://chat.whatsapp.com/..."
                  className="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-xl text-white font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all shadow-inner"
                />
              </div>
              
              {/* VIP Toggle - Only show if mode allows or default */}
              <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-amber-500/10 to-yellow-500/10 border border-amber-500/20 rounded-xl">
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    checked={newPackage.isVip}
                    onChange={(e) => setNewPackage({ ...newPackage, isVip: e.target.checked })}
                    className="sr-only peer"
                  />
                  <div className="w-11 h-6 bg-white/10 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-amber-500"></div>
                </label>
                <div>
                  <p className="text-amber-400 font-bold text-sm flex items-center gap-2">
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                    </svg>
                    VIP Package
                  </p>
                  <p className="text-amber-500/70 text-[10px]">Enable for referral rewards & VIP benefits</p>
                </div>
              </div>

              <div className="flex gap-4 mt-8">
                <button
                  type="button"
                  onClick={() => setShowAddModal(false)}
                  className="flex-1 px-4 py-4 bg-white/5 hover:bg-white/10 text-white font-black uppercase tracking-widest text-xs border border-white/10 rounded-xl transition-all"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className={`flex-1 px-4 py-4 font-black uppercase tracking-widest text-xs rounded-xl transition-all shadow-lg ${
                    newPackage.isVip
                      ? 'bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-black shadow-amber-500/30'
                      : 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-600/30'
                  }`}
                >
                  {editingId ? 'Update Pack' : 'Create Pack'}
                </button>
              </div>
            </form>
          </div>
        </div>
      )}

      {/* Members Modal */}
      {viewingMembersPkg && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl flex items-center justify-center z-[110] p-4 text-left">
             <div className="bg-slate-900 border border-white/10 rounded-3xl max-w-2xl w-full max-h-[80vh] flex flex-col shadow-2xl overflow-hidden">
                <div className="p-6 border-b border-white/10 flex items-center justify-between bg-white/5">
                     <div>
                        <h3 className="text-xl font-black text-white">{viewingMembersPkg.name} Members</h3>
                        <p className="text-slate-400 text-xs">Total: {members.length}</p>
                     </div>
                     <button onClick={() => { setViewingMembersPkg(null); setMembers([]); }} className="p-2 hover:bg-white/10 rounded-lg">
                        <svg className="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                     </button>
                </div>
                
                <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                    {loadingMembers ? (
                        <div className="py-10 text-center">
                            <div className="w-8 h-8 border-4 border-white/10 border-t-blue-500 rounded-full animate-spin mx-auto"></div>
                            <p className="text-slate-500 text-xs mt-2">Loading members...</p>
                        </div>
                    ) : members.length === 0 ? (
                        <div className="py-10 text-center">
                            <p className="text-slate-500">No members found in this package</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            {members.map((member: any) => (
                                <div key={member._id} className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/5">
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-full bg-blue-500/20 text-blue-400 flex items-center justify-center font-bold text-xs">
                                            {member.name.charAt(0)}
                                        </div>
                                        <div>
                                            <p className="font-bold text-white text-sm">{member.name}</p>
                                            <p className="text-slate-400 text-xs font-mono">{member.phoneNumber}</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                         <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Registered</p>
                                         <p className="text-xs text-white">{new Date(member.createdAt).toLocaleDateString()}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    )}
                </div>
             </div>
        </div>
      )}
    </div>
  );
}
